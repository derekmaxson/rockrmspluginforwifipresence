...
namespace RockWeb.Plugins.WifiPresence
{
    [DisplayName( "Wi-Fi Presence Captive Portal" )]
    [Category( "Plugins > Wi-Fi Presence" )]
    ...
    [Rock.SystemGuid.BlockTypeGuid( "B31D5874-C9CE-48AB-B3B7-0B94C352A495" )]
    public partial class CaptivePortal : RockBlock
    {
        // Extracted from RockPage so direct device linking is owned by this plugin.
        private bool LinkPersonAliasToDevice( int personAliasId, string macAddress )
        {
    ...
    <ol>
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            // Query strings remain on the WebForms postback URL. Validate on every request,
            // before device/person changes, and never fall back to a configured release link.
            if ( !ValidateReleaseUrl() )
            {
                base.OnLoad( e );
                return;
            }
            ...
            if ( !IsPostBack )
            {
                ...
                if ( CurrentPerson != null )
                {
                    ...
                    LinkPersonAliasToDevice( ( int ) CurrentPersonAliasId, macAddress );
                }
                ...
                if ( !ShowControls() )
                {
                    ...
                    {
                        string redirectUrl = CreateRedirectUrl( CurrentPersonAliasId );
                        nbAlert.Text = string.Format( "If you did not have Administrative permissions on this block you would have been redirected to: <a href=\"{0}\">{1}</a>.",
                            HttpUtility.HtmlAttributeEncode( redirectUrl ), HttpUtility.HtmlEncode( redirectUrl ) );
                        nbAlert.Visible = true;
                    }
                    else
                    {
        ...
        {
            if ( !ValidateReleaseUrl() || !ValidateForm() )
            {
                return;
            }
        ...
        protected int? LinkDeviceToPerson()
        {
            ...
            if ( tbFirstName.Visible && tbLastName.Visible && ( tbMobilePhone.Visible || tbEmail.Visible ) )
            {
                ...
                if ( person != null )
                {
                    LinkPersonAliasToDevice( person.PrimaryAlias.Id, hfMacAddress.Value );
        ...
        /// <summary>
        /// Builds the release URL from the current request's fppostback parameter.
        /// </summary>
        /// <param name="primaryAliasId">if null then no id parameter is appended to the returned URL</param>
        /// <returns></returns>
        protected string CreateRedirectUrl( int? primaryAliasId )
        {
            Uri releaseUri;
            if ( !TryGetReleaseUri( Request.QueryString, out releaseUri ) )
            {
                throw new InvalidOperationException( "Missing or invalid fppostback release URL." );
            }

            return BuildReleaseUrl( Request.QueryString["fppostback"], Request.ServerVariables["QUERY_STRING"], primaryAliasId );
        }

        private bool ValidateReleaseUrl()
        {
            Uri releaseUri;
            if ( TryGetReleaseUri( Request.QueryString, out releaseUri ) )
            {
            ...
            nbAlert.Text = "Missing or invalid fppostback. Supply one absolute HTTP or HTTPS release URL in the query string.";
            nbAlert.Visible = true;
            ShowControls( false );
            return false;
        }

        #region Release URL helpers
        // These helpers use only .NET types so their request/redirect contract can be tested
        // without loading Rock or changing its database.
        private static bool TryGetReleaseUri( NameValueCollection queryString, out Uri releaseUri )
        {
            releaseUri = null;
            string[] values = queryString.GetValues( "fppostback" );
            if ( values == null || values.Length != 1 || string.IsNullOrWhiteSpace( values[0] ) )
            {
                return false;
            }

            // Request.QueryString has already decoded the outer query string once.
            // Decoding again would corrupt escaped values within the callback URL.
            string value = values[0];
            if ( value.Any( char.IsControl ) || value.Contains( "\\" )
                || !Uri.TryCreate( value, UriKind.Absolute, out releaseUri ) )
            {
                return false;
            }

            return ( releaseUri.Scheme == Uri.UriSchemeHttp || releaseUri.Scheme == Uri.UriSchemeHttps )
                && !string.IsNullOrWhiteSpace( releaseUri.Host )
                && string.IsNullOrEmpty( releaseUri.UserInfo );
        }

        private static string BuildReleaseUrl( string releaseUrl, string rawQueryString, int? primaryAliasId )
        {
            // Keep both query strings as raw text: parsing/re-encoding would change
            // parameter order, repeated keys, percent-escape casing and plus/space encoding.
            int fragmentIndex = releaseUrl.IndexOf( '#' );
            string fragment = fragmentIndex < 0 ? string.Empty : releaseUrl.Substring( fragmentIndex );
            string target = fragmentIndex < 0 ? releaseUrl : releaseUrl.Substring( 0, fragmentIndex );
            string forwardedQuery = rawQueryString ?? string.Empty;

            if ( primaryAliasId.HasValue )
            {
                // Append only; do not rename, remove or overwrite any inbound parameter.
                forwardedQuery += ( forwardedQuery.Length == 0 || forwardedQuery.EndsWith( "&" ) ? "" : "&" )
                    + "id=" + primaryAliasId.Value.ToString( System.Globalization.CultureInfo.InvariantCulture );
            }
            ...
            if ( forwardedQuery.Length > 0 )
            {
                ...
                target += separator + forwardedQuery;
            }
